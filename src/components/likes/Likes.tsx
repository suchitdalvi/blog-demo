...
import { LIKES_END_POINT } from "../../shared/constants/app.constants";
...
export default function Likes({
  ...
  const [userLike, setUserLike] = useState<PostLike>();
  ...
  useEffect(() => {
    if (postId) {
      setApiObject({
        url: `${LIKES_END_POINT}?postId=${postId}`,
        method: "GET",
      });
    }
  }, [postId]);
  ...
  useEffect(() => {
    if (data) {
      if (apiObject?.method === "GET") {
        setPostLikes(data);
        setUserLike(
          [...data].find((like: PostLike) => like.userId === currentUserId)
        );
      } else if (apiObject?.method === "POST" && data["id"]) {
        setUserLike(data);
        setPostLikes((prevLikes) => {
          const allLikes = [...prevLikes];
          allLikes.push(data);
          return allLikes;
        });
      } else if (apiObject?.method === "DELETE") {
        setUserLike(undefined);
        setPostLikes((prevLikes) => {
          return [...prevLikes].filter((like) => like.userId !== currentUserId);
        });
      }
    }
  ...
  const handleLike = (): void => {
    if (!currentUserId) {
      ...
      if (userLike) {
        setApiObject({
          url: `${LIKES_END_POINT}/${userLike.id}`,
          method: "DELETE",
        });
      } else {
        setApiObject({
          url: `${LIKES_END_POINT}`,
          method: "POST",
          optionBody: { postId, userId: currentUserId },
        });
      }
    }
  };
  ...
  return (
    <div className="flex">
      <Button
        classes=" "
        text={userLike ? alreadyLikedImage : likeImage}
        onClick={() => handleLike()}
        disabled={loading}
      />{" "}
      {postLikes?.length ?? 0}{" "}
